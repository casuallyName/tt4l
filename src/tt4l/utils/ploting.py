...
class LogHistory(BaseModel):
    ...
    def add(self, step: int, value: Union[int, float]):
...
class Logs(BaseModel):
    ...
    def add(self, key: str, step: int, value: Union[int, float]):
        if key not in self._logs.keys():
            self._logs[key] = LogHistory(key=key)
            setattr(self, key, self._logs[key])
        self._logs[key].add(step, value)

    @property
    def plots(self) -> Dict[str, "matplotlib.figure.Figure"]:
        return {key: getattr(self, key).plot for key in self.keys}
...
class LogsHistory(BaseModel):
    ...
    @property
    def plots(self) -> Dict[str, "matplotlib.figure.Figure"]:
        train_plots = {f'train.{key}': getattr(self.train, key).plot for key in self.train.keys}
        eval_plots = {f'eval.{key}': getattr(self.eval, key).plot for key in self.eval.keys}
        return {**train_plots, **eval_plots}
            ...
            if 'grad_norm' in log:
                for key in log.keys():
                    if key not in IGNORE_LOG_KEYS:
                        logs.train.add(key, log[STEP_KEY_NAME], log[key])
            elif 'eval_runtime' in log:
                for key in log.keys():
                    if key not in IGNORE_LOG_KEYS:
                        logs.eval.add(key, log[STEP_KEY_NAME], log[key])
