...
# @About    :
__all__ = ['multi_label_categorical_cross_entropy_loss', 'MultiLabelCategoricalCrossEntropyLoss']
...
# class TokenClassificationMultiLabelCategoricalCrossEntropyLoss(_Loss):
#     def __init__(self, batch_size, ent_type_size, size_average=None, reduce=None, reduction: str = 'mean') -> None:
#         super(TokenClassificationMultiLabelCategoricalCrossEntropyLoss, self).__init__(size_average, reduce, reduction)
#         self.reshape_size = batch_size * ent_type_size
#
#     def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
#         inputs = inputs.reshape(self.reshape_size, -1)
#         targets = targets.reshape(self.reshape_size, -1)
#
#         return multi_label_categorical_cross_entropy(inputs, targets, reduction=self.reduction)
